	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			if(multiRequest.getOriginalFileName("reUpfile") != null) {
				...
				if(multiRequest.getParameter("origibalFileNo") != null) {
			...
			int result = new PostService().updateNoticePost(p, at);
			
			if(result > 0) {
				
				request.getSession().setAttribute("alertMsg", "게시글이 수정되었습니다!");
				
				response.sendRedirect(request.getContextPath() + "/nDetail.no?pno=" + postNo);
