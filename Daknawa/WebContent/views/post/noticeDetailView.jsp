...
<body>
    ...
    <br>
        ...
        <div align="center">
            <a href="nlist.no?currentPage=1" class="btn btn-secondary btn-sm">목록</a>
				
              <% if(loginUser != null && loginUser.getUserNickName().equals(p.getMemberNickname())) { %>
                <a href="<%= contextPath %>/nupdateForm.no?pno=<%= p.getPostNo() %>" class="btn btn-warning btn-sm">수정하기</a>
                <a href="<%= contextPath %>/ndelete.no?pno=<%= p.getPostNo() %>" class="btn btn-danger btn-sm">삭제하기</a>
              <% } %> 
        </div>
